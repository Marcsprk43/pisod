...
def draw_altitude(frame, mv):
    ...
    cv2.putText(frame, 'A:{:.40f}'.format(mv.data['Altitude']*3.28), (550, 25), font, 1, text_color, 2, cv2.LINE_AA)

def draw_lat_lon(frame, mv):
    cv2.putText(frame, 'Lat:{:02.6f}'.format(int(mv.data['Lat'])), (5, 25), font, 1, (0,255, 255), 2, cv2.LINE_AA)
    cv2.putText(frame, 'Lon:{:02.6f}'.format(int(mv.data['Lon'])), (280, 25), font, 1, (0,255, 255), 2, cv2.LINE_AA)
